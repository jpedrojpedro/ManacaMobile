...
import br.com.tag.mobile.imgConvert.ImageToSDCard;
															  ...
															  OnItemClickListener
{
	...
	public void onScrollStateChanged(AbsListView view, int scrollState)
	{
		...
		if ( scrollState == SCROLL_STATE_IDLE )
		{
			...
			for ( int i = firstPosition; i <= lastPosition; i++ )
			{
				...
				{
					Product p = (Product) view.getItemAtPosition(i);
					Bitmap img = ImageToSDCard.getImage(p.getImageName());
					if ( img == null )
					{
						Log.d("ImageNameScroll", "Image from SDCard: " + p.getImageName());
						new GetProductsThumbTask(this, p.getImageName(), i).execute();
					}
					else
					{
						Log.d("ImageNameScroll", "Image from WEB: " + p.getImageName());
						this.setThumbImage(i, img);
					}
					/*
					String imgEncoded = 
	...
	public void setThumbImage ( int position, Bitmap img )
	{
		try
		{
			...
			p.img.setImageBitmap(img);
			Log.d("InsertThumb", "Posição: " + position);
			ImageToSDCard.storeImage(p.imgName, img);
			/*
		...
		{
			Log.e("InsertThumb", "NULL POINTER: " + e.toString());
		}
	}
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3)
	{
		...
		i.putExtra("productPrice", Float.parseFloat(p.amount.getText().toString()));
